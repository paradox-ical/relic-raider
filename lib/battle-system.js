...
class BattleSystem {
  ...
  static async executePlayerAction(battleState, action) {
    ...
    // Optimized cooldown reduction - batch operations
    const cooldowns = battleState.actionCooldowns;
    if (cooldowns.special > 0) cooldowns.special--;
    if (cooldowns.defend > 0) cooldowns.defend--;
    
    // Optimized skill cooldown reduction
    ...
    // Regenerate energy and ultimate progress
    ...
    const playerStatusEffects = battleState.playerStatusEffects;
    if (playerStatusEffects) {
      const expiredEffects = [];
      for (const [effectType, effect] of Object.entries(playerStatusEffects)) {
      ...
      expiredEffects.forEach(effectType => {
        delete playerStatusEffects[effectType];
      });
    }
  ...
  static executePlayerAttack(battleState) {
    // Optimized passive skill effects - cache the system
    ...
    // Optimized status effects processing
    const WeaponEffectsSystem = require('./weapon-effects-system');
    const statusEffects = WeaponEffectsSystem.processStatusEffects(battleState);
    
    // Calculate attack with optimized parameters
    ...
    battleState.battleLog.push(battleLogEntry);
  ...
  // Get available player actions (optimized with caching)
  static async getPlayerActions(userId) {
    try {
      // Use cached actions if available to reduce database calls
      if (global.cachedPlayerActions && global.cachedPlayerActions[userId]) {
        const cached = global.cachedPlayerActions[userId];
        if (Date.now() - cached.timestamp < 5000) { // Cache for 5 seconds
          return cached.actions;
        }
      }
      
      // Optimized database queries - combine into single query where possible
      const [equippedSkills, userSkills] = await Promise.all([
        prisma.equippedSkill.findMany({
          where: { userId: userId },
          include: { skill: true },
          orderBy: { slot: 'asc' }
        }),
      ...
      for (const equippedSkill of equippedSkills) {
        if (equippedSkill.skill.type === 'ACTIVE' && equippedSkill.slot >= 1 && equippedSkill.slot <= 4) {
          const userSkill = userSkillMap.get(equippedSkill.skillId);
      ...
      if (ultimateSkill) {
        const userSkill = userSkillMap.get(ultimateSkill.skillId);
        const level = userSkill ? userSkill.level : 1;
      ...
      // Cache the results
      if (!global.cachedPlayerActions) {
        global.cachedPlayerActions = {};
      }
      global.cachedPlayerActions[userId] = {
        actions: actions,
        timestamp: Date.now()
      };
